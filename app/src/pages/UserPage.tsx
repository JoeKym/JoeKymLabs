          ...
          <div className="space-y-8">
              ...
              <div className="space-y-3 text-sm text-[#A6ACB8]">
                ...
                {userProfile?.website && (
                  <div className="flex items-center gap-3">
                    ...
                    <a href={userProfile.website.startsWith('http') ? userProfile.website : `https://${userProfile.website}`} target="_blank" rel="noreferrer" className="text-[#B8B2F7] hover:underline">
            ...
            <div className="flex items-center gap-6 pb-6 border-b border-white/10">
              <div className="flex flex-col">
                <span className="font-bold text-lg text-white">{userProfile?.followers_count || 0}</span>
                <span className="text-xs text-[#A6ACB8] font-mono uppercase tracking-wider">Followers</span>
              </div>
              <div className="flex flex-col">
                <span className="font-bold text-lg text-white">{userProfile?.following_count || 0}</span>
            ...
            {(userProfile?.github || userProfile?.twitter) && (
              <div>
                ...
                <div className="flex gap-3">
                  {userProfile.github && (
                    <a href={`https://github.com/${userProfile.github}`} target="_blank" rel="noreferrer" className="p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors text-[#A6ACB8] hover:text-white">
                      <Github size={20} />
                    </a>
                  )}
                  {userProfile.twitter && (
                    <a href={`https://twitter.com/${userProfile.twitter}`} target="_blank" rel="noreferrer" className="p-3 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors text-[#A6ACB8] hover:text-[#1DA1F2]">
                      <Twitter size={20} />
                    </a>
                  )}
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
                <div className="text-2xl font-display font-bold text-white mb-1">{projects.length}</div>
                <div className="text-[10px] text-[#A6ACB8] font-mono uppercase tracking-wider">Projects</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
                <div className="text-2xl font-display font-bold text-white mb-1">{activities.length}</div>
                <div className="text-[10px] text-[#A6ACB8] font-mono uppercase tracking-wider">Contributions</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
                <div className="text-2xl font-display font-bold text-[#B8B2F7] mb-1">{totalLikes}</div>
                <div className="text-[10px] text-[#A6ACB8] font-mono uppercase tracking-wider">Likes Received</div>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 text-center">
                <div className="text-2xl font-display font-bold text-white mb-1">{userProfile?.reputation_points || 0}</div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {projects.length > 0 ? (
                    projects.map((project) => (
                      <div key={project.id} className="group bg-white/5 border border-white/10 rounded-2xl overflow-hidden hover:border-white/20 transition-all">
                        <div className="h-32 bg-black/40 w-full relative overflow-hidden">
                          {project.image_url ? (
                            ...
                            <div className="absolute inset-0 bg-gradient-to-br from-[#B8B2F7]/20 to-transparent" />
                          )}
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                          <div className="absolute bottom-3 left-3 flex gap-2 flex-wrap">
                            {(project.tags || []).map(tag => (
                              <span key={tag} className="px-2 py-1 bg-white/10 backdrop-blur-md rounded text-[10px] font-mono uppercase text-white">{tag}</span>
                            ))}
                          </div>
                        </div>
                        <div className="p-4">
                          <h3 className="font-semibold text-lg mb-1 group-hover:text-[#B8B2F7] transition-colors">{project.title}</h3>
                          <p className="text-sm text-[#A6ACB8] line-clamp-2 mb-4">{project.description}</p>
                    ...
                    <div className="sm:col-span-2 p-8 border border-dashed border-white/10 rounded-2xl text-center text-[#A6ACB8]">
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {activities.length > 0 ? (
                    activities.map((activity) => (
                      <div key={activity.id} className="flex gap-4 p-4 bg-white/5 border border-white/10 rounded-2xl">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                          activity.activity_type === 'like' ? 'bg-red-500/10 text-red-400' : 'bg-[#B8B2F7]/10 text-[#B8B2F7]'
                        }`}>
                        ...
                        <div>
                          <p className="text-sm text-[#F7F8FA]">
                            ...
                            <span className="text-[#B8B2F7] cursor-pointer">{activity.target_name}</span>
                          </p>
                          {activity.content && (
                            <p className="text-sm text-[#A6ACB8] mt-1 p-3 bg-black/20 rounded-xl italic">"{activity.content}"</p>
                          )}
                          <p className="text-xs text-[#A6ACB8] mt-2 font-mono">
                    ...
                    <div className="p-8 border border-dashed border-white/10 rounded-2xl text-center text-[#A6ACB8]">
