  ...
  return (
    <section className="relative h-screen w-full overflow-hidden z-10 flex flex-col justify-center">
      <div className="absolute inset-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-slate-50/40 via-slate-50/10 to-slate-50/85" />
      </div>
      <div className="relative h-full flex flex-col justify-center px-6 lg:px-[6vw] animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300 fill-mode-both">
        <div className="max-w-xl">
          <p className="font-mono text-xs tracking-[0.2em] uppercase text-indigo-600 mb-4">Web Design &amp; Development Studio</p>
          <h1 className="font-display font-bold text-5xl md:text-6xl lg:text-7xl text-slate-900 leading-[0.95] mb-6">JoeKym Labs™</h1>
          <p className="text-base md:text-lg text-slate-700 leading-relaxed mb-8 max-w-md">
            We craft clean, editorial interfaces and build them with performant code — for brands that value clarity over noise.
          </p>
          <div className="flex flex-wrap gap-4">
            <Link to="/work" className="px-6 py-3 bg-indigo-600 text-white font-medium rounded-xl hover:scale-105 transition-transform">View selected work</Link>
            <a href="#contact" className="px-6 py-3 border border-slate-200 text-slate-900 font-medium rounded-xl hover:bg-slate-100 transition-colors">Start a project</a>
          </div>
        </div>
      </div>
...
function WorkSection() {
  return (
    <section id="work" className="relative bg-slate-50 py-24 z-20">
      <div className="flex flex-col lg:flex-row px-6 lg:px-[6vw] gap-12">
        <div className="w-full lg:w-[32vw] flex flex-col justify-center animate-in fade-in slide-in-from-left-8 duration-1000">
          <h2 className="font-display font-bold text-4xl md:text-5xl lg:text-6xl text-slate-900 mb-4">Selected Work</h2>
          <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-6 max-w-xs">A few recent builds—minimal UI, strong typography, and motion that respects the content.</p>
          <Link to="/work" className="text-indigo-600 text-sm font-medium hover:underline">Browse archive →</Link>
        </div>
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="group relative w-full aspect-[4/3] rounded-[22px] overflow-hidden card-shadow animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-100">
            ...
            <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end"><h3 className="font-display font-semibold text-lg text-slate-900">Lumen Editorial</h3><span className="font-mono text-xs text-slate-600 uppercase">Website</span></div>
          </div>
          <div className="group relative w-full aspect-[4/3] rounded-[22px] overflow-hidden card-shadow animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-200">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-slate-50/80 to-transparent" />
            <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end"><h3 className="font-display font-semibold text-lg text-slate-900">Northwind Goods</h3><span className="font-mono text-xs text-slate-600 uppercase">E‑commerce</span></div>
          </div>
        </div>
      </div>
    </section>
...
function CapabilitiesSection() {
  ...
  return (
    <section id="studio" className="relative bg-slate-50 py-24 z-30">
      <div className="flex flex-col px-6 lg:px-[6vw]">
        <div className="w-full mb-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <h2 className="font-display font-bold text-4xl md:text-5xl lg:text-6xl text-slate-900 mb-4">Capabilities</h2>
          <p className="text-slate-600 text-sm md:text-base leading-relaxed max-w-xs">Strategy, design, and frontend engineering—built to ship.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {capabilities.map((cap, i) => (
            <div key={cap.title} className="group relative w-full aspect-[3/4] rounded-[22px] overflow-hidden card-shadow animate-in fade-in slide-in-from-bottom-8 duration-1000" style={{ animationDelay: `${i * 150}ms` }}>
              ...
              <div className="absolute bottom-6 left-6 right-6"><h3 className="font-display font-semibold text-2xl text-slate-900 mb-2">{cap.title}</h3><p className="text-slate-600 text-sm">{cap.desc}</p></div>
            </div>
          ))}
        </div>
      </div>
...
function ProcessSection() {
  ...
  return (
    <section className="relative bg-slate-50 py-24 z-40">
      <div className="flex flex-col px-6 lg:px-[6vw]">
        <div className="w-full mb-12 animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <h2 className="font-display font-bold text-4xl md:text-5xl lg:text-6xl text-slate-900 mb-4">Process</h2>
          <p className="text-slate-600 text-sm md:text-base leading-relaxed max-w-xs">A simple loop: understand, design, build, refine.</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {processes.map((proc, i) => (
            <div key={proc.title} className="group relative w-full aspect-[3/4] rounded-[22px] overflow-hidden card-shadow animate-in fade-in slide-in-from-bottom-8 duration-1000" style={{ animationDelay: `${i * 150}ms` }}>
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-50 via-slate-50/40 to-transparent" />
              <div className="absolute bottom-6 left-6 right-6"><h3 className="font-display font-semibold text-2xl text-slate-900 mb-2">{proc.title}</h3><p className="text-slate-600 text-sm">{proc.desc}</p></div>
            </div>
          ))}
        </div>
      </div>
...
function SplitSection({ id, microLabel, title, body, cta, ctaLink = '#', image, imagePosition = 'left', zIndex = 50 }: { id?: string; microLabel: string; title: string; body: string; cta: string; ctaLink?: string; image: string; imagePosition?: 'left' | 'right'; zIndex?: number; }) {
  return (
    <section id={id} className="relative bg-slate-50 flex flex-col lg:flex-row py-12 lg:py-0 min-h-[70vh]" style={{ zIndex }}>
      ...
      <div className="w-full lg:w-[50vw] bg-slate-50 flex flex-col justify-center px-6 lg:px-24 py-16 lg:py-24">
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000">
          <p className="font-mono text-xs tracking-[0.08em] uppercase text-indigo-600 mb-4">{microLabel}</p>
          <h2 className="font-display font-bold text-3xl md:text-4xl lg:text-5xl text-slate-900 leading-tight mb-6">{title}</h2>
          <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-8 max-w-md">{body}</p>
          <a href={ctaLink} className={`inline-block px-6 py-3 font-medium rounded-xl transition-all ${['FEATURED PROJECT', 'COLLABORATION', 'CRAFT', 'COMMUNITY'].includes(microLabel.toUpperCase()) ? 'bg-indigo-600 text-white hover:scale-105' : 'border border-slate-200 text-slate-900 hover:bg-slate-100'}`}>{cta}</a>
        </div>
      </div>
...
function ContactSection() {
  ...
  return (
    <section id="contact" className="relative bg-slate-50 py-32 z-50">
      <div className="max-w-4xl mx-auto px-6 text-center animate-in fade-in slide-in-from-bottom-8 duration-1000">
        <h2 className="font-display font-bold text-5xl md:text-7xl text-slate-900 mb-8">Let's build.</h2>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a href="mailto:mail.jkyme@gmail.com" className="w-full sm:w-auto px-8 py-4 bg-indigo-600 text-white font-bold rounded-xl hover:scale-105 transition-transform text-lg">
            Start a project
          </a>
          {!user && (
            <Link to="/auth" className="w-full sm:w-auto px-8 py-4 border border-slate-200 text-slate-900 font-bold rounded-xl hover:bg-slate-100 transition-colors text-lg">
              Client Login
            </Link>
          )}
          {user && (
            <Link to="/profile" className="w-full sm:w-auto px-8 py-4 border border-slate-200 text-slate-900 font-bold rounded-xl hover:bg-slate-100 transition-colors text-lg">
              Dashboard
            </Link>
          )}
        </div>
...
export default function LandingPage() {
  return (
    <div className="bg-slate-50 text-slate-900 font-sans selection:bg-indigo-600 selection:text-indigo-900 overflow-x-hidden">
      <Navigation />
      <HeroSection />
      <SplitSection 
        id="collab"
