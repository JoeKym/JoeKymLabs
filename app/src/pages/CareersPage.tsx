    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32 pb-20">
        ...
        <section className="mb-20">
          ...
          {loading ? (
            <div className="text-center text-slate-600 animate-pulse">Querying HRIS API / Database...</div>
          ) : jobs.length > 0 ? (
            <div className="space-y-4">
              {jobs.map((job, idx) => (
                <div key={job.id} className="bg-white shadow-sm border border-slate-200 rounded-2xl p-6 hover:bg-slate-100 transition-colors animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${idx * 100}ms` }}>
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                    <div>
                      <h3 className="text-xl font-bold text-slate-900">{job.title}</h3>
                      <div className="flex items-center gap-3 mt-2 text-sm font-mono text-slate-600">
                        <span className="bg-slate-50 border border-slate-200 px-2 py-1 rounded">{job.department}</span>
                        <span>•</span>
                    ...
                    <button className="px-6 py-2 bg-indigo-600 text-white font-bold rounded-xl hover:scale-105 transition-transform">
                      Apply Now
                    </button>
                  </div>
                  <p className="text-sm text-slate-600 mb-4">{job.description}</p>
                  <div className="text-xs text-indigo-600 font-mono">Salary Band: {job.salary_band}</div>
                </div>
              ))}
            </div>
          ) : (
