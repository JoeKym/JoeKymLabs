    ...
    <div className="min-h-screen bg-[#07080A] text-[#F7F8FA] font-sans">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32 pb-20">
        <header className="mb-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          <p className="text-[#A6ACB8] font-mono text-sm">Last updated: {new Date().toLocaleDateString()}</p>
        </header>

        <div className="flex flex-col md:flex-row gap-12">
          <aside className="w-full md:w-48 shrink-0">
            <nav className="flex flex-row md:flex-col gap-2 overflow-x-auto pb-4 md:pb-0">
              <button onClick={() => setActiveTab('privacy')} className={`text-left px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap transition-colors ${activeTab === 'privacy' ? 'bg-white/10 text-white' : 'text-[#A6ACB8] hover:text-white'}`}>Privacy Policy</button>
              <button onClick={() => setActiveTab('terms')} className={`text-left px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap transition-colors ${activeTab === 'terms' ? 'bg-white/10 text-white' : 'text-[#A6ACB8] hover:text-white'}`}>Terms of Service</button>
              <button onClick={() => setActiveTab('cookies')} className={`text-left px-4 py-2 rounded-lg font-medium text-sm whitespace-nowrap transition-colors ${activeTab === 'cookies' ? 'bg-white/10 text-white' : 'text-[#A6ACB8] hover:text-white'}`}>Cookie Policy</button>
            </nav>
          </aside>

          <div className="flex-1 prose prose-invert prose-p:text-[#A6ACB8] prose-h2:font-display prose-h2:text-2xl prose-h2:font-bold prose-h2:text-[#F7F8FA] prose-h2:mt-8 prose-h2:mb-4 prose-p:leading-relaxed prose-a:text-[#B8B2F7] animate-in fade-in duration-500">
            {activeTab === 'privacy' && (
              <div>
            ...
            {activeTab === 'cookies' && (
              <div>
                ...
                <div className="mt-8 border border-white/10 rounded-xl overflow-hidden bg-black/20">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-white/5 font-mono text-xs uppercase text-[#A6ACB8]">
                      <tr>
                        <th className="px-6 py-3 border-b border-white/10">Name</th>
                        <th className="px-6 py-3 border-b border-white/10">Purpose</th>
                        <th className="px-6 py-3 border-b border-white/10">Duration</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5 text-[#A6ACB8]">
                      <tr>
                        <td className="px-6 py-4 font-mono text-white">sb-access-token</td>
                        <td className="px-6 py-4">Authentication state for Supabase</td>
                        <td className="px-6 py-4">Session</td>
                      </tr>
                      <tr>
                        <td className="px-6 py-4 font-mono text-white">_ga</td>
                        <td className="px-6 py-4">Google Analytics tracking</td>
                        <td className="px-6 py-4">2 years</td>
                      </tr>
                      <tr>
                        <td className="px-6 py-4 font-mono text-white">theme_pref</td>
